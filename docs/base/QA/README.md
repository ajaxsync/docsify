...
数组的创建：
    ...
    + 内置构造函数：
        ...
        - var arr = new Array(10) // 创建一个长度为10位的数组
        - var arr = new Array(10, 56, 34) // 创建一个有内容的数组
        - var arr = new Array('10') // 创建一个值是字符串10的数组
...
数组的索引：
    - 索引也叫下标，是指一个数据在数组中排在哪一个位置
    - 注意: 在所有语言里面，索引都是从0开始
    - 获取数组的某个元素：
        * 获取数组的第一个元素是 arr[0] 
        * 每个数组的最后一个元素是 arr[length-1]
    ...
    + 两种数据类型的赋值
        - 简单数据类型
            * 举例：
              ...
              // 将a1的值复制了一份给a2，但两者互不影响
              // 即使更改了a1的值为20，a2的值依旧是10
        ...
        - 复杂数据类型
            * 举例：
            ...
            console.log(arr1==arr2)  // true  因为两者保存的是同一个地址

            * 比喻：
                - 相当于你租了一个房子，然后你同学想与你合租
                - 于是你把自己的钥匙配了一把给他
                - 所以你们都可以打开这个房子
                - 因此，无论是你还是你同学，若是改变了房间里的摆设
                - 那么，当另外一个人回来的时候，房间的摆设就变化了
    ...
    + 两种数据类型作为函数参数
        ...
        - 复杂数据类型
            * 举例：
                ...
                function fn2(n){
                    n[n.length] = "hello" // 3、n得到的是m的地址，也会改变m的值
                }
                fn2(m);  //  2、m保存的是地址，所以是将地址复制了一份给n
                console.log(m);  // 4、n改变时，会影响m的值 [1,2,3,4, "hello"]
